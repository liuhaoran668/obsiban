...
1. 在调用函数时检查对应的一个生命周期，避免出现没有定义的行为，在调用hand.angle.set_angle的时候将使用ensure检查对象是否已经析构。
```
...
2. 回调函数（**读取的一个高频操作，所以需要使用线程池**）使用多线程模式，设计线程池进行并行处理回调函数，回调函数不会阻塞，如果订阅较多的话，会进行排队处理。
	1. 前一个版本是在一个线程内进行**接收消息和处理回调**，所有的回调都在一个线程中进行，如果一个回调处理慢，会导致其他的回调延迟很长。
	2. 在当前版本中，将两者进行拆分，让接收线程只接收消息，剩下的交给线程池，将消息放入队列，将给线程池去处理，这样所有的操作都是实时的，一个回调的延迟也只是自己的manager进行延迟。不会影响其他。
		1. **总结就是之前的版本一个慢回调就可能会阻塞后续所有的回调或者消息接收。** 
```
  recv_thread_ 的工作循环：
  ┌──────────────────────────────────────────────────────────┐
  │  while (running) {                                        │
  ...
  │    ② read() 读取 CAN 帧                                   │
  │    ③ for (sub : subscribers) {                            │
  │         sub->queue.push_back(msg);  ← 只入队，纳秒级操作   │
  │         submit_task(drain(sub));    ← 提交给线程池         │
  │       }                                                    │
  │    // ③ 瞬间完成，立刻回到 ①                                │
  │  }                                                        │
  └──────────────────────────────────────────────────────────┘
  ...
  线程池 (2-4 个 worker)：
  ┌──────────────────────────────────────────────────────────┐
  │  Worker 1: drain(A) → [A回调 5ms] [A回调 5ms] ...        │
  │  Worker 2: drain(B) → [B回调 2000ms...............] ...   │
  │  Worker 3: drain(C) → [C回调 5ms] [C回调 5ms] ...
```

4. 在手的close函数中使用原子变量进行进行避免竞态多次执行close，同时手的析构函数也是调用对应的close。close函数对其中的功能模块进行析构。
