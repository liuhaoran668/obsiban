...
git的一些原始概念：分支指针，head指针，tree，commit，blob

其的一个指向链路：head指针->分支指针->commit->tree->blob
1. head指针：是总体的一个指向，指向分支。。告诉查看现在在什么地方，在哪个分支（相册）----其的作用就是查看方向，head指向的是提交
2. 分支指针：就是类似于书签，在哪个相册，新开一个分支就是将当前的提交给新开一个相册内容
3. commit类比相册，里面装着原信息，不是真正的东西，是一些具体内容的指针，包括作者和提交时间
4. tree就相当于是目录，里面有多个具体的blob，没有改变的blob会共享。每次提交都会有tree。
5. blob存储着具体的信息。

总体的一些命令就是去改变这些指针的指向来实现版本管理。




git rm -r --cached build/   --这个命令是将已经跟踪的文件进行消除跟踪。
